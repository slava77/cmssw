...
DEFINE_FWK_MODULE(IterMultiClusterProducer);

IterMultiClusterProducer::IterMultiClusterProducer(const edm::ParameterSet &ps) :
...
void IterMultiClusterProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  // hgcalMultiClusters
  ...
  descriptions.add("iterHGCalMultiClusters", desc);
}

void IterMultiClusterProducer::produce(edm::Event& evt, const edm::EventSetup& es) {

    edm::Handle<std::vector<reco::BasicCluster> > clusterHandle;
    edm::Handle<std::vector<reco::BasicCluster> > clusterSharingHandle;
    edm::Handle<std::vector<bool> > inputClustersMask_h;

    evt.getByToken(clusters_token, clusterHandle);
    evt.getByToken(clustersMask_token, inputClustersMask_h);
    for( auto elt : *inputClustersMask_h)
    {
